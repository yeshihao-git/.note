...
| 模型空间 / 局部空间   | 模型坐标        |                                                |
| 世界空间          | 世界坐标        | 3D                                             |
| 相机空间 / 视图空间   | 相机坐标 / 视图坐标 | 视锥体是相机空间中的一个可见区域<br>**投影变换是将相机空间中的视锥体变换到剪裁空间** |
| 裁剪空间 / 齐次坐标空间 | 裁剪坐标 / 齐次坐标 | 齐次坐标 (x, y, z, w) w != 1；；x、y、z坐标范围[-w, w]     |
| 标准化设备空间（NDC）  | 标准化设备坐标     | 齐次坐标 (x/w, y/w, z/w, 1)；x、y、z坐标范围[-1, 1]       |
| 屏幕窗口空间        | 屏幕像素坐标      | 2D；[0, 窗口视口宽] x [0, 窗口视口高]                     |

在 opengl 和 webgl 中，==透视除法是 GPU 自动做的一个步骤==

**透视除法**：
```ts
// 公式的核心是：把四维裁剪坐标 clip(x, y, z, w) 转换成三维 NDC 坐标 ndc(x, y, z)
ndc.x = clip.x / clip.w
ndc.y = clip.y / clip.w
ndc.z = clip.z / clip.w
// clip.w 的值不是规定的，但常用的透视投影矩阵选择让 clip.w = -camera.z
```
...
视图矩阵 对应 LookAt 函数（https://zhuanlan.zhihu.com/p/66240124）
